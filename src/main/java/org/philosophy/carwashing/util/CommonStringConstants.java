package org.philosophy.carwashing.util;

public class CommonStringConstants {
    public static final String USER_NOT_FOUND_MESSAGE = "пользователь не найден по id, обратитесь в техподдержку";
    public static final String WASH_TYPE_NOT_FOUND_MESSAGE = "услуга не найдена по id";
    public static final String REQUEST_NOT_FOUND_MESSAGE = "запрос бронирования с таким Id не найден";
    public static final String BOX_NOT_FOUND_MESSAGE = "Бокс с таким Id не найден";
    public static final String BOX_TYPE_NOT_FOUND_MESSAGE = "Тип бокса с таким Id не найден";
    public static final String ENTITY_NOT_FOUND_MESSAGE = "Сущность не найдена. ";
    public static final String WRONG_PARAMETERS_MESSAGE = "Неверные параметры. ";
    public static final String WRONG_DATE_SEQUENCE = "Неверный порядок времени, время До не может быть раньше времени От.";
    public static final String NULL_FIELDS_IN_REQUEST_DTO = "null fields in request dto";
    public static final String DELETING_NOT_ALOWED = "Удаление невозможно, сущность связана с другими сущностями ";
    public static final String NOTING_FOUND = "По вашему запросу ничего не найдено, измените параметры поиска";
    public static final String ACCESS_TOKEN = "access_token";
    public static final String REFRESH_TOKEN = "refresh_token";
    public static final String ROLES = "roles";
    public static final String USERNAME = "username";
    public static final String PASSWORD = "password";

}
